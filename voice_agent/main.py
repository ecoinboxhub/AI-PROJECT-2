...
# Make imports resilient whether running as package or as a script
sys.path.insert(0, str(parent_dir))
try:
    # Preferred: package import when running from repository root
    from voice_agent.app import create_voice_app
    from voice_agent.config import VoiceConfig
except Exception:
    # Fallback: direct module import when running inside the package folder
