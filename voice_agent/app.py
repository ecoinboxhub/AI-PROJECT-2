...
try:
    # Preferred when running from repository root or as a package
    from voice_agent.dialog.agent_router import AgentRouter
    from voice_agent.dialog.greeting import get_greeting_text
    from voice_agent.dialog.prompt_processor import PromptProcessor
    from voice_agent.realtime.stt_whisper import WhisperSTT
    from voice_agent.realtime.tts_elevenlabs import ElevenLabsTTS
    from voice_agent.analytics.quality_metrics import QualityMetrics
    from voice_agent.config import VoiceConfig
except Exception:
    # Fallback when running inside the `voice_agent` folder (module mode)
...
def create_voice_app(config: VoiceConfig) -> FastAPI:
    ...
    @app.post("/api/v1/outbound/session-prepare")
    async def prepare_outbound_session(body: Dict[str, Any]):
        """
        Prepare an outbound voice session. This endpoint does NOT originate
        the call. It prepares greeting audio, seeds session state, and
        returns session metadata for the backend to use when dialing.
        """
        try:
            ...
            message_content = body.get("message_content") or ""
            personalization = body.get("personalization") or {}
            tts_options = body.get("tts_options") or {}

            if not business_name or not str(business_name).strip():
                raise ValueError("business_name is required for greeting")

            # Generate greeting text using existing helper
            reason = None
            if trigger_type == "technical_fixed":
                issue = (body.get("context") or {}).get("issue_summary") or personalization.get("issue_summary") or "an issue we fixed"
                greeting_text = f"Good day, this is {business_name}. We fixed {issue}. Would you like a summary?"
            elif trigger_type == "order_located":
                order_id = (body.get("context") or {}).get("order_id") or personalization.get("order_id") or "your order"
                status = (body.get("context") or {}).get("status") or "located"
                greeting_text = f"Good day, this is {business_name}. Your order #{order_id} has been located and is {status}."
            else:
                offer = (body.get("context") or {}).get("offer") or personalization.get("offer") or "a new offer"
                expiry = (body.get("context") or {}).get("expiry") or personalization.get("expiry")
                if expiry:
                    greeting_text = f"Good day, this is {business_name}. You qualify for {offer} valid until {expiry}."
                else:
                    greeting_text = f"Good day, this is {business_name}. You qualify for {offer}."

            # Synthesize greeting audio
            t0 = time.perf_counter()
            audio_bytes = await tts_service.synthesize_speech(
                text=greeting_text,
                output_format=tts_options.get("format", "MP3")
            )
            tts_latency = int((time.perf_counter() - t0) * 1000)
            ...
            active_sessions[session_id] = {
                ...
                "created_at": datetime.now().isoformat(),
                "conversation_history": [
                    {"system": message_content, "timestamp": datetime.now().isoformat()}
                ],
                "turn_count": 0
            }
            ...
            if config.ENABLE_METRICS:
                quality_metrics.record_turn(
                    ...
                    turn_count=0,
                    transcript_length=0,
                    processing_time=tts_latency/1000.0,
                )

            resp = {
                "session_id": session_id,
                "greeting": {"text": greeting_text, "audio_base64": audio_b64},
            ...
            logger.error(f"Error preparing outbound session: {str(e)}")
            raise HTTPException(status_code=500, detail=str(e))
