...
class VectorStoreManager:
    ...
    def _init_chroma(self):
        ...
        try:
            ...
            # Monkey-patch chromadb's default embedding to avoid onnxruntime
            import sys
            import importlib
            
            # Create a fake onnxruntime to prevent DLL errors
    ...
    def _init_memory(self):
        """Initialize simple in-memory vector store (no persistence)"""
        try:
            ...
            # Simple in-memory storage: dict of business_id -> list of (text, embedding, metadata)
            self.memory_store = {}
            logger.info("In-memory vector store initialized successfully (no persistence)")
            
        except Exception as e:
            logger.error(f"Error initializing in-memory store: {e}")
            raise
    ...
    def _init_pgvector(self):
        ...
        try:
            ...
            logger.warning("Falling back to in-memory mode (no persistence). Data will be lost on restart.")
            # Use simple in-memory storage as fallback
            self.storage_mode = "memory"
            self._init_memory()
            logger.info("Fallback to in-memory mode succeeded. Vector store running without persistence.")
    ...
    def _ensure_table_exists(self, business_id: str):
        ...
        try:
            with conn.cursor() as cur:
                ...
                # IMPORTANT: Operator class must match the query operator for index utilization:
                #   <-> operator: L2/Euclidean distance, uses vector_l2_ops
                #   <=> operator: Cosine distance, uses vector_cosine_ops (standard pgvector)
                #   <#> operator: Negative inner product, uses vector_ip_ops
                # Queries use <=> operator for cosine distance, so index uses vector_cosine_ops
                # If <=> computes L2 in your pgvector version, change to vector_l2_ops
    ...
    def add_documents(
        ...
        try:
            if self.storage_mode == "memory":
                # In-memory mode
                if business_id not in self.memory_store:
                    self.memory_store[business_id] = []
                ...
                for doc_id, doc_text, embedding, metadata in zip(ids, documents, embeddings, metadatas):
                    self.memory_store[business_id].append({
                        'id': doc_id,
                        'document': doc_text,
                        'embedding': embedding,
                        'metadata': metadata
                ...
                logger.info(f"Added {len(documents)} documents to {business_id} (in-memory)")
                return True
    ...
    def query(
        ...
        try:
            if self.storage_mode == "memory":
                # In-memory mode
                if business_id not in self.memory_store or not self.memory_store[business_id]:
                    logger.warning(f"No documents found for {business_id}")
                ...
                import numpy as np
                results = []
                for doc in self.memory_store[business_id]:
                    # Calculate cosine distance (1 - cosine similarity)
                    doc_embedding = np.array(doc['embedding'])
                    query_emb = np.array(query_embedding)
                    cosine_sim = np.dot(doc_embedding, query_emb) / (np.linalg.norm(doc_embedding) * np.linalg.norm(query_emb))
...
def get_vector_store(storage_mode: Optional[str] = None) -> VectorStoreManager:
    ...
    return VectorStoreManager(storage_mode=storage_mode)
