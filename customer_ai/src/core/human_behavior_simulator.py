...
class HumanBehaviorSimulator:
    ...
    def _initialize_personalities(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            'nigerian_business': {
                'greeting_style': ['Good morning!', 'Hello!', 'Good afternoon!', 'Good evening!', 'Hi!'],
                'acknowledgments': ['Got it!', 'Sure!', 'No problem!', 'Okay!', 'Understood!'],
                'closing': ['Thanks!', 'No problem!', 'You\'re welcome!', 'Talk soon!', 'Have a great day!'],
                'thinking': ['Let me check...', 'One moment...', 'Hmm...', 'Let me look into that...', 'Let me look...'],
                'typo_frequency': 0.01,      # 1% chance of casual typo
                'emoji_usage': 0.05,         # 5% chance of emoji
                'exclamation_usage': 0.20,   # 20% chance of !
                'checking_frequency': 0.25    # 25% chance to "check"
            },
    ...
    def _detect_communication_style(self, customer_query: str) -> str:
        ...
        Pidgin is only detected when the customer uses 3+ unmistakable Pidgin words.
        """
        try:
            query_lower = customer_query.lower()
            
            # Pidgin indicators — only clear, unmistakable Pidgin words
            pidgin_words = ['wetin', 'abeg', 'no wahala', 'sha', 'na so', 'dey', 'oya']
            pidgin_count = sum(1 for word in pidgin_words if word in query_lower)
            ...
            # Pidgin requires 3+ unmistakable indicators to activate
            if pidgin_count >= 3:
                return 'informal'  # Treat heavy pidgin as informal, NOT pidgin
            elif formal_count >= 1:
                return 'formal'
    ...
    def add_nigerian_context(self, response: str, use_slang: bool = False) -> str:
        """
        Add Nigerian cultural context to response.
        Only handles currency symbol normalisation.
        """
        try:
    ...
    def _extract_yes_no_with_detail_complete(self, response: str, communication_style: str) -> str:
        ...
        try:
            ...
            if any(word in response.lower() for word in ['yes', 'sure', 'available']):
                ...
                if communication_style == 'formal':
                    return "No, delivery is not available at the moment."
                else:  # informal
                    return "No, we don't deliver there."
            else:
                # Fallback: return first 8 words
                words = response.split()[:8]
                return ' '.join(words)
