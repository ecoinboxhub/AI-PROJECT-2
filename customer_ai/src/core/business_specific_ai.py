...
After the LangGraph migration, response generation is handled by the
LangGraph agent (customer_agent.py). This module is used for:
  - train_with_business_data / update_business_data  (onboarding)
  - generate_faqs_from_business_data
  - generate_ai_context_from_minimal_data
  - _analyze_intent / _analyze_sentiment  (standalone API endpoints)
...
class BusinessSpecificAI:
    ...
    def _normalize_business_data(self, business_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized = {
            ...
            "products_services": business_data.get("products", []) + business_data.get("services", []),
    ...
    def generate_faqs_from_business_data(self) -> List[Dict[str, str]]:
        """Generate FAQs automatically from business data using AI."""
        try:
            if not self.openai_api_key:
            ...
            faq_prompt = f"""
Based on the following business information, generate 8-12 relevant FAQs that customers would commonly ask:

Business: {business_context['business_name']}
Industry: {business_context['industry']}
Description: {business_context['description']}
Products/Services: {json.dumps(business_context['products_services'], indent=2)}
Contact Info: {json.dumps(business_context['contact_info'], indent=2)}

Generate FAQs that cover:
1. General business information
2. Products/services details
3. Pricing and availability
4. Contact and location
5. Policies and procedures (industry-specific)
6. Common customer concerns

Return as a JSON object: {{"faqs": [{{"question": "...", "answer": "..."}}]}}.
"""
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "You are an expert at generating relevant FAQs for businesses. "
                            "Provide helpful, accurate answers based on the business information provided. "
            ...
            try:
                ...
                return self._extract_faqs_from_text(faq_text)

        except Exception as e:
            logger.error(f"Error generating FAQs: {str(e)}")
            return self._get_fallback_faqs()

    def _get_fallback_faqs(self) -> List[Dict[str, str]]:
        business_name = self._get_business_name()
        return [
            {"question": f"What services does {business_name} offer?", "answer": "Please contact us directly for detailed information about our services."},
            {"question": f"How can I contact {business_name}?", "answer": "Please check our contact information for phone, email, and address details."},
            {"question": "What are your business hours?", "answer": "Please contact us to confirm our current business hours."},
            {"question": "Do you offer consultations or appointments?", "answer": "Please contact us to schedule a consultation or appointment."},
        ]

    def _extract_faqs_from_text(self, text: str) -> List[Dict[str, str]]:
        """Extract FAQs from unstructured text when JSON parsing fails."""
        ...
        for line in lines:
            ...
            if line.endswith("?") and (
                line.startswith("Q:") or line.startswith("Question:")
                or any(w in line.lower() for w in ["what", "how", "when", "where", "why", "do you", "can you"])
            ):
                ...
                line.startswith("A:") or line.startswith("Answer:")
                or (current_answer and not line.startswith("Q:") and not line.startswith("Question:"))
            ):
        ...
        return faqs if faqs else self._get_fallback_faqs()
...
"""
    ...
    def _analyze_sentiment(self, message: str) -> Dict[str, Any]:
        """Analyze customer sentiment from message (used by /analyze_sentiment endpoint)."""
        try:
            if not self.openai_api_key:
...
"""
    ...
    def _analyze_intent(self, message: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """Analyze customer intent from message (used by /extract_intent endpoint)."""
        try:
            if not self.openai_api_key:
